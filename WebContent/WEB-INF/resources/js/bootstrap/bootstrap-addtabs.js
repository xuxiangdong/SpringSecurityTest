...
$.fn.addtabs = function (options) {
    ...
    obj.on('click', '.close-tab', function () {
	 ...
	 obj.on('click', '#close_all', function () {
       var allTabs = $('.nav-tabs').find('*[aria-controls]');
	   for(var i=0;i<allTabs.length;i++)
	   {
		   if(console)console.info($(allTabs[i]).attr("aria-controls"));
			Addtabs.close($(allTabs[i]).attr("aria-controls"));
	   }
    });
...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        if (!$("#" + id)[0]) {
            ...
            if (Addtabs.options.close) {
                title.append(
                    //$('<i >',{'class':'close-tab glyphicon glyphicon-remove'})
                		$('<i class="close-tab glyphicon glyphicon-remove"> </li>')
                );
            }
            //创建新TAB的内容
            var content = $('<div>', {
            ...
            //加入TABS
			 var closeAll = $('.nav-tabs').find("#close_all").parent();
			//if(closeAll == undefined || closeAll.length=0)
			//{
			//	obj.children('.nav-tabs').append(title);
           // }
			//else
			//{
				closeAll.before(title);
			//}
    ...
    drop: function () {
        ...
        element.append(dropdown.find('li'))
            ...
            //	if (this.offsetTop > 55 || element.width() - $(this).position().left - $(this).width() < 53) {
